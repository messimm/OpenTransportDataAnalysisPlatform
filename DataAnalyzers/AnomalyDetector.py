...
class AnomalyDetection(BasicDataAnalysisModule):
    def __init__(self, cfg):
        ...
        self.night_thr = cfg.get("night_threshold", 0)
    ...
    def _load_holidays(self):
        try:
            ...
            return pd.to_datetime(response.json().get("holidays", [])).date
        except Exception:
            return set()
    ...
    def detect_anomalies(self, data):
        ...
        for _, row in data.iterrows():
            ...
            history = history.sort_values("date").tail(self.N_history)
            val = float(history[self.detection_column].mean())

            if self.night_flag and pd.to_datetime(self.night_start).time() < current_time < pd.to_datetime(self.night_finish).time():
            ...
            typical.append(val)
            if self.mode == "value":
                anomaly.append(abs(row[self.detection_column] - val) > self.thr + night_correction)
            else:
                anomaly.append(abs(row[self.detection_column] - val) > self.thr * val + night_correction)

        return typical, anomaly
    ...
    def analyze(self, data_loader, data_checker):
        ...
        my_data = my_data[[self.ts, self.detection_column]].copy()
        typical, anomaly = self.detect_anomalies(my_data)
        my_data["typical"] = typical
        my_data["anomality"] = anomaly
        my_data = my_data[[self.ts, self.detection_column, "typical", "anomality"]]

        graphic = my_data[[self.ts, self.detection_column]].copy()
        graphic.columns = ["x", "y"]
        points = graphic[anomaly]
